...
function connect2MySQL(){
    try{
        $pdo = new PDO('mysql:host=127.0.0.1;dbname=challenge_db;charset=utf8','root');
        // 失敗時に PDOException 例外が投げられるようにする
        $pdo -> setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
        return $pdo;
    } catch (PDOException $connectE) {
        die('接続に失敗しました。次記のエラーにより処理を中断します:'.$connectE->getMessage());
    }
}
function A($name,$birthday,$tell,$type,$comment){
