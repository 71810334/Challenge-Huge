...
<body>
    ...
    <select name="year">
        <option value="----">----</option>
        <?php
        // 「年」に値が入っていれば、その年を表示(課題4)
        if ($rec_year != null) {echo '<option selected value="' . $rec_year . '">' . $rec_year . '</option>';}
?>
<?php
        for($i=1950; $i<=2010; $i++){ ?>
...
<?php
        // 「月」に値が入っていればその月を表示(課題4)
            if ($rec_month != null) {echo '<option selected value="' . $rec_month . '">' . $rec_month . '</option>';}
?>
<?php        
        for($i = 1; $i<=12; $i++){?>
        <option value="<?php echo $i;?>"><?php echo $i;?></option>
...
<?php
        // 「日」に値が入っていればその日を表示(課題4)
            if ($rec_day != null) {echo '<option selected value="' . $rec_day . '">' . $rec_day . '</option>';}
?>
<?php        
        for($i = 1; $i<=31; $i++){ ?>
        <option value="<?php echo $i; ?>"><?php echo $i;?></option>
    ...
    <?php if ($rec_type == 1){echo 'checked';} ?>>エンジニア<br>
    <input type="radio" name="type" value="2"
    <?php if ($rec_type == 2){echo 'checked';} ?>>営業<br>
    <input type="radio" name="type" value="3"
    <?php if ($rec_type == 3){echo 'checked';} ?>>その他<br>
    <br>
    ...
    <?php echo $rec_comment; ?></textarea><br><br>
